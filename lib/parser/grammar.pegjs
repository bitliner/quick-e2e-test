...
start = scenario / beforeEach

scenario = "when" space actor space a:actions "," space "then" space c:conditions space* { return {actions:a, conditions: c, type:'scenario'} }/actor space a:preAction {return a}
beforeEach="before each" space as:actions {return {type:"beforeEach", actions:as}}
...
clickAction=a:click space "on" space o:string  {return {name:a, obj: o, type:'action'} }
sendAction=a:send space o:string  space "to" space l:string  {return {name:a,obj:o, location:l, type:'action'} }
goAction="go" space "to" space s:string {return {name:"go",obj:s, type:'goAction'}}
anyAction=digits:[A-z\-]+ {return {name: digits.join(''),type:'macro'} }
